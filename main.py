...
from models.Update import LocalUpdate, LocalUpdateCACS, LocalUpdateCACS_NC
...
#     # 7번과 13번을 제외한 코어 할당
#     cpu_list = list(range(16))  # 0-15 코어
...
if __name__ == '__main__':
    ...
    print(f'''Starting training:
        ...
        args.novel_class_num    {args.novel_class_num}
    ...
    for iter in range(args.epochs + 1):
        ...
        for ind, idx in enumerate(idxs_users):
            start_in = time.time()
            if args.epochs == iter:
                if args.alg == 'fedrep' or args.alg == 'lg' or args.alg == 'fedper':
                    # print("fedrep")
                    local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users_train[idx])
                elif args.alg == 'fedcacs':
                    # print("fedcacs")
                    local = LocalUpdateCACS(args=args, dataset=dataset_train, idxs=dict_users_train[idx])
                elif args.alg == 'fedcacsNC':
                    # print("fedcacsNC")
                    local = LocalUpdateCACS_NC(args=args, dataset=dataset_train, idxs=dict_users_train[idx])
                else:
        ...
        if args.alg == 'fedrep' or args.alg == 'fedcacs' or args.alg == 'fedper':
            if iter > args.epochs - 4:
                ...
                for key in head_weights_keys:
                    if 'bias' not in key and glob_head is not None: # 이 부분에 glob_head를 검사하는 조건 추가
                        if args.cos:
        ...
        if (iter + 1) % args.test_freq == 0:
            ...
            print('Round {:3d}, Train loss: {:.3f}, glob_loss: {:.3f}, glob_cor: {:.2f}, all_local_loss: {:.2f}, '
                'all_local_cor: {:.2f}, Time: {}'
                .format(iter, loss_avg, glob_loss, glob_cor, all_local_loss, all_local_cor, current_time))
        ...
        if iter == args.epochs:
            ...
            os.makedirs(os.path.dirname(model_save_path), exist_ok=True)  # 이 부분 추가
    ...
    # 저장
    plt.savefig(f'./result/' + args.alg + '_' + args.dataset + str(args.num_users) + '_' + str(args.shard_per_user) + '.png')
    plt.show()