...
def headavg(args, local_nets_dict, head_weights_keys, user_cat_index):
    ...
    for idx in range(args.num_users):
        ...
        for key in head_weights_keys:
            for line in cate_set:
                if 'bias' not in key and line in cate_weight_component_dict:
                    if args.cos:
    ...
    for idx in range(dict_length):
        if len(cate_weight_component_dict[idx]) > 0:
    ...
    if args.cos and glob_head is not None:
        glob_head = glob_head.transpose(0, 1)
    elif glob_head is None:
        glob_head = torch.zeros(1, dict_length).cuda()  # <-- 이 부분을 추가
    return glob_head
