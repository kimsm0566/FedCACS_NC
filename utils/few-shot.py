...
from models.novelUpdate import finetuneLocalUpdate, finetuneLocalUpdateWithNC
...
if __name__ == '__main__':
    ...
    for user in trange(args.novel_users):
        if args.alg == 'fedcacs_nc' :
            pre_updater = finetuneLocalUpdateWithNC(args=args, dataset=dataset_train, idxs=dict_users_train[user])
            novel_updater = finetuneLocalUpdateWithNC(args=args, dataset=dataset_train, idxs=dict_users_train[user])       
        else : 
